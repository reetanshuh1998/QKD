...
### 2. The Autoencoder Hybrid (The Winning Architecture)
...
*   These anomalies are horizontally piped backward into an independently tuned **Gradient Boosting (XGBoost)** classification algorithm.
...
Inside the `models/plots/` repository, the automated pipeline generates empirical verifications:
...
*   `confusion_matrix.png`: Absolute class separation mapping demonstrating overlapping vectors and false positives corresponding inherently to highly identical physical signatures (e.g., RNG bias vs Wavelength bias).
*   `roc_curves.png`: Receiver Operating Characteristic limits evaluating the hybridized algorithm across all 8 tracking modes.
*   `learning_curve.png`: Evaluation loss profiles monitoring the tree cascade boundaries across sequential validation rounds, tracking hyperparameter generalization.
