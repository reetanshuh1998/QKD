...
\midrule
Standalone XGBoost & 86.92\% \\
Hybrid (AE + XGBoost) & \textbf{88.18\%} \\
Deep Neural Network & 89.44\% \\
\bottomrule
\end{tabular}
\end{table}

The DNN achieves the highest raw accuracy at 89.44\%. The hybrid model attains 88.18\%, closing the gap to within one percentage point while providing two critical advantages for operational QKD deployment: (1)~SHAP-based interpretability enabling operators to verify which physical observables triggered a detection, and (2)~structural adversarial resilience demonstrated in Section~\ref{sec:adversarial}.
...
\midrule
Pre-evasion (baseline) & 94.13\% \\
Post-evasion (40-epoch gradient attack) & 57.47\% \\
\bottomrule
\end{tabular}
\end{table}
