...
In practice, however, QKD systems use attenuated laser sources producing Weak Coherent Pulses (WCP), which follow Poisson photon-number statistics and occasionally emit multi-photon pulses. An adversary can exploit these multi-photon events via Photon-Number Splitting (PNS) attacks~\cite{brassard2000limitations}, extracting key information without measurably increasing the QBER. Additional hardware-layer attacks include detector blinding through bright illumination~\cite{lydersen2010hacking}, Trojan-horse probing of the transmitter~\cite{gisin2002quantum}, and timing-channel exploits targeting detection-efficiency mismatches~\cite{qi2007time}.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{models/plots/attack_signatures_across_observables.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{models/plots/hybrid_feature_importance.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{models/plots/roc_curves.png}}
...
Before evasion, the hybrid model detects attack traffic at its baseline rate. After gradient-based optimization, the adversarially modified samples reduce the autoencoder's MSE substantially. However, the XGBoost classifier---operating on the full hybrid feature vector including both raw features and latent representations---maintains its detection capability because:
...
The decoy-state BB84 protocol was introduced by Lo et al.~\cite{lo2005decoy} and Ma et al.~\cite{ma2005practical} to close the PNS vulnerability in WCP-based QKD. Hardware attacks on commercial QKD systems have been extensively documented, including detector blinding~\cite{lydersen2010hacking}, time-shift attacks~\cite{qi2007time}, and Trojan-horse attacks~\cite{gisin2002quantum}. ML-based intrusion detection for QKD has been explored using neural networks and ensemble methods, though typically on simplified feature sets lacking decoy-state observables. Adversarial robustness of tree-based classifiers was analyzed by Chen et al.~\cite{chen2019robust}. Our work bridges these domains by combining physics-faithful simulation with an adversarially aware hybrid architecture.
...
The complete source code, including the decoy-state BB84 simulator, attack injection modules, training pipelines, and figure generation scripts, is publicly available at \texttt{[repository URL]}. The dataset is fully reproducible from the published code via \texttt{python3 src/generate\_qkd\_dataset.py --iters 5000 --pulses 10000} with seed~42.
